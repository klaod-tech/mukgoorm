...
Discord_Damagotchi/
├── bot.py                    # 봇 메인 진입점
├── database.py               # DB 초기화 진입점 (utils/db.py 하위호환용)
├── CLAUDE_PROMPT.md          # Claude 새 대화 시작용 컨텍스트 프롬프트
├── cogs/
...
├── images/                   # [진행중] 25종 이미지 배치 — 아래 P1 참고
├── models/                   # [미생성] ML 모델 저장 (.pkl) — ML 구현 시 생성 예정
├── docs/
│   └── CONTEXT.md            # 프로젝트 컨텍스트 문서 (현 파일)
├── .env                      # 환경변수
└── requirements.txt
```
...
[P1] 다마고치 이미지 25종 배치 [진행중]
  - image.py 참조 기준 파일명: eating.png / snowy.png / dusty.png (언더스코어, 소문자)
  - 배치 완료 (4종): normal.png, hot.png, rainy.png, tired.png
  - 파일명 수정 필요 (3종): eat.png→eating.png, snow.png→snowy.png, wear mask.png→dusty.png
  - 미배정 (4종): cheer.png, smile.png, upset.png, warm.png (IMAGE_TRIGGER_TABLE 어느 상태에 쓸지 미정)
  - 미배치 (나머지): happy.png, sick.png, hungry.png, hungry_cry.png, overfed.png, underfed.png,
                     cold.png, sunny.png, cloudy.png, goal_achieved.png, birthday.png
  - 배치 완료 후 utils/image.py 실제 파일로 테스트
