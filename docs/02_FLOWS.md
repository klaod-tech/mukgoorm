...
| 발화 예시 | 감지 의도 | 처리 흐름 | 최종 응답 위치 |
|-----------|-----------|-----------|-----------|
| "점심에 비빔밥 먹었어" | meal | 메인봇이 🍽️ 식사입력 버튼 제공 → 클릭 후 텍스트/사진 입력 | 유저 전용 채널 |
| 사진 직접 업로드 | meal_photo | 식사봇 on_message 감지 → "음식 사진이에요?" 버튼 표시 | 유저 전용 채널 |
| "몸무게 기록할게, 68kg" | weight | task_queue → 체중봇 처리 → 메인봇에 결과 반환 | 유저 전용 채널 (메인봇 Embed 갱신) |
| "오늘 하루 어떻게 지냈는지 적고 싶어" | diary | task_queue → 일기봇 처리 → 메인봇에 결과 반환 | 유저 전용 채널 (메인봇 Embed 갱신) |
| "다음 주 월요일 팀 미팅 있어" | schedule | task_queue → 일정봇 처리 → 메인봇에 결과 반환 | 유저 전용 채널 + 알림 쓰레드 (D-day) |
...
#먹구름-시작 채널의 [🐣 다마고치 시작하기] 클릭
  ...
  → 전용 채널 안에 쓰레드 생성 (총 2개 — Push 알림 전용):
      1. 🔔 {이름}의 알림      → info_thread_id  (날씨 매일 기상 + 일정 D-day)
      2. 📧 {이름}의 메일함    → mail_thread_id  (메일봇 전용)

  → 전용 채널에 캐릭터 Embed 고정 (pin) + embed_message_id 저장
  → SchedulerCog.register_meal_jobs(user_id) 호출
  → WeatherCog: info_thread_id 기준 wake_time Job 등록
...
```
[예상 흐름]
자연어: "오늘 하루 적고 싶어" → 먹구름봇 감지 → task_queue INSERT → 일기봇 poll
  또는 /일기작성 슬래시 커맨드
...
```
[예상 흐름]
자연어: "다음 주 월요일 병원 예약 있어" → 먹구름봇 감지 → task_queue INSERT → 일정봇 poll
  또는 /일정등록 슬래시 커맨드
  → GPT-4o 날짜/시간 파싱
  → DB schedules 저장
