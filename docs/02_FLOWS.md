...
> 이미지는 `file=img_file`로 첨부 전송 → embed 위에 크게 표시됨. `set_thumbnail()` 미사용.

### 버튼 동작

| 버튼 | 처리 봇 | 동작 | 출력 위치 |
|------|---------|------|-----------|
| 🍽️ 식사 입력 | 먹구름봇 (텍스트) / 식사봇 (사진) | 텍스트 → MealInputModal / 사진 → DB 대기 상태 → 식사봇 감지 | 메인 쓰레드 |
| 📋 하루 정리 | 먹구름봇 | Ephemeral: 칼로리·탄단지·끼니·체중·날씨·GPT 7개 필드 | Ephemeral |
| 🍜 뭐 먹고 싶어? | 먹구름봇 | "준비 중" (n8n 연동 예정) | Ephemeral |
| ⚙️ 설정 | 먹구름봇 | SettingsSubView: 내정보/위치/시간/이메일 하위 메뉴 | Ephemeral |
| ⚖️ 체중 기록 | 먹구름봇* | WeightInputModal → DB 저장 → 목표 달성 판정 | Ephemeral + 체중 쓰레드** |

> *체중 기록은 현재 `cogs.weight`가 bot.py에 임시 로드됨. bot_weight.py 분리 예정.  
> **현재는 Ephemeral만 전송. 체중관리봇 분리 후 `weight_thread_id` 쓰레드 공개 전송으로 전환 예정.
...
```
/일정등록 → ScheduleInputModal (제목, 날짜, 시간, 반복)
  → DB schedules 저장
...
```
유저 메시지 → 먹구름봇 on_message
  → GPT 의도 분석 → task_queue 테이블 삽입
  → 각 전문봇 30초 폴링 → 태스크 처리
예: "오늘 비빔밥 먹었어"
  → 식사봇 트리거 (비빔밥 분석)
  → 일기봇 트리거 (오늘 기록 유도)
