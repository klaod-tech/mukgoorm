...
## 수정 2 — `취향 정리` 노드 Softmax 정렬로 교체

### 목적
음식 추천 시 단순 랜덤/순서 출력 대신, 유저의 학습된 선호도를 반영한 Softmax 확률 기반 정렬로 교체합니다.

### 현재 흐름 (수정 전)
```
유저 정보1 → 취향 정리 → 식당1 → 5개 출력 → ...
```

### 목표 흐름 (수정 후)
```
유저 정보1 → 로짓 전체 조회 → 취향 정리(교체) → 식당1 → 5개 출력(교체) → ...
```

---

### Step 1 — 로짓 전체 조회 노드 삽입

`유저 정보1`과 `취향 정리` 사이에 새 노드를 삽입합니다.

1. `유저 정보1` → `취향 정리` 연결선 클릭해서 선택 후 **Delete**로 연결 끊기
2. `유저 정보1` 노드의 **+** 버튼 클릭
3. `Supabase` 노드 추가 후 설정:
...
4. 이름: `로짓 전체 조회`
5. 이 노드의 출력을 `취향 정리` 노드에 연결 (드래그로 선 연결)

---

### Step 2 — `취향 정리` 노드 코드 교체
